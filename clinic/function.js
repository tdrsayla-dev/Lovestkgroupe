...
// Immediate cleanup of bloated base64 or oversized entries in localStorage on script load
(function cleanupBloatedLocalStorage() {
    try {
        ['clinic_lab_files_meta', 'clinic_services_packages', 'clinic_commission_logs', 'clinic_bills_cache', 'clinic_visits_queue'].forEach(k => {
            try {
...
// =====================================
// Centralized Real-time Synchronization Hub (All Tables)
// =====================================
let _realtimeDebounceTimers = {};

function debounceRealtime(key, callback, delay = 250) {
    if (_realtimeDebounceTimers[key]) {
        clearTimeout(_realtimeDebounceTimers[key]);
    }
    _realtimeDebounceTimers[key] = setTimeout(() => {
        try {
            callback();
        } catch (e) {
            console.warn('Realtime callback error for ' + key, e);
        }
    }, delay);
}

function initClinicRealtimeHub() {
    if (typeof _supabase === 'undefined' || !_supabase) return;

    try {
        console.log('⚡ Initializing Clinic Central Real-time Subscriptions across all tables...');

        // 1. Channel สำหรับตาราง VISITS (คัดกรอง, ตรวจแพทย์, แล็บ, จัดคิว, อ่านผล/จัดยา, จ่ายยา, ชำระเงิน, ประวัติ)
        _supabase.channel('realtime_visits')
            ...
            .subscribe((status) => {
                if (status === 'SUBSCRIBED') console.log('✅ Realtime Active: visits table');
            });

        // 2. Channel สำหรับตาราง APPOINTMENTS (นัดหมายล่วงหน้า & ปฏิทิน Dashboard)
        _supabase.channel('realtime_appointments')
            .on('postgres_changes', { event: '*', schema: 'public', table: 'appointments' }, (payload) => {
                console.log('⚡ [Realtime: Appointments Update]', payload.eventType);
                debounceRealtime('appointments', () => {
                    if (typeof loadAppointments === 'function') loadAppointments();
                    if (typeof window.renderCalendar === 'function') window.renderCalendar();
                    if (typeof updateDashboardStats === 'function') updateDashboardStats();
                }, 200);
            ...
            .subscribe((status) => {
                if (status === 'SUBSCRIBED') console.log('✅ Realtime Active: appointments table');
            });

        // 3. Channel สำหรับตาราง PATIENTS (ทะเบียนผู้ป่วย)
        _supabase.channel('realtime_patients')
            ...
            .subscribe((status) => {
                if (status === 'SUBSCRIBED') console.log('✅ Realtime Active: patients table');
            });

        // 4. Channel สำหรับตาราง MEDICINES (คลังยา & สต็อกยา)
        _supabase.channel('realtime_medicines')
            .on('postgres_changes', { event: '*', schema: 'public', table: 'medicines' }, (payload) => {
                console.log('⚡ [Realtime: Medicines Update]', payload.eventType);
                debounceRealtime('medicines', () => {
                    if (typeof loadStockList === 'function') loadStockList();
                    if (typeof loadMedicines === 'function') loadMedicines();
                    if (typeof populateRxMedDropdown === 'function') populateRxMedDropdown();
                }, 200);
            })
            .subscribe((status) => {
                if (status === 'SUBSCRIBED') console.log('✅ Realtime Active: medicines table');
            });

        // 5. Channel สำหรับตาราง SUPPLIES & SUPPLY_REQUESTS (คลังพัสดุ & เบิกพัสดุ)
        _supabase.channel('realtime_supplies')
            .on('postgres_changes', { event: '*', schema: 'public', table: 'supplies' }, () => {
                debounceRealtime('supplies', () => {
                    if (typeof loadSupplyItems === 'function') loadSupplyItems();
                }, 200);
            })
            .on('postgres_changes', { event: '*', schema: 'public', table: 'supply_requests' }, () => {
                debounceRealtime('supply_requests', () => {
                    if (typeof loadSupplyRequests === 'function') loadSupplyRequests();
                }, 200);
            })
            .subscribe();
        ...
        // 8. Channel สำหรับตาราง SERVICES (รายการบริการตรวจ)
        _supabase.channel('realtime_services')
            .on('postgres_changes', { event: '*', schema: 'public', table: 'services' }, () => {
                debounceRealtime('services', () => {
                    if (typeof loadServicesData === 'function') loadServicesData();
                }, 200);
            })
            .subscribe();
...
async function loadTriage() {
    // โหลดประวัติการคัดกรองเบื้องต้น (ตารางล่าง) ควบคู่ไปด้วยเสมอ
...
function switchBillsView(view) {
    ...
    if (view === 'daily') {
        ...
        if (startInput && !startInput.value) {
            startInput.value = getExpenseLocalDateStr(new Date());
        }
        if (endInput && !endInput.value) {
            endInput.value = getExpenseLocalDateStr(new Date());
        }
...
async function submitAppointment() {
    ...
    try {
        if (selectedRefBy) {
            ...
            if (typeof window.safeSetLocalStorage === 'function') {
                window.safeSetLocalStorage('clinic_appointment_referrers', window.appointmentReferrersMap);
            } else {
                localStorage.setItem('clinic_appointment_referrers', JSON.stringify(window.appointmentReferrersMap));
            }
        ...
        console.warn('clinic_appointment_referrers LocalStorage error:', ex);
    }
...
async function submitPatient() {
    ...
    try {
        if (refByVal) {
            ...
            if (typeof window.safeSetLocalStorage === 'function') {
                window.safeSetLocalStorage('clinic_patient_referrers', window.patientReferrersMap);
            } else {
        ...
        console.warn('clinic_patient_referrers LocalStorage error:', ex);
    }
...
async function submitPrescription() {
    ...
    // 🌟 ดึงอาการแรกรับเดิม (ถ้ามี) ไม่ให้โดนทับด้วยคำว่า 'สั่งจ่ายยา'
    let originalSymptom = '';
    if (window.clinicVisits) {
        const v = window.clinicVisits.find(x => x.visit_id === visitId);
    ...
    try {
        ...
        if (vIndex !== -1) {
            ...
            if (originalSymptom) {
                cachedVisits[vIndex].symptom = originalSymptom;
            }
        ...
        // ⚠️ จำกัดข้อมูลไว้ที่ 50 แถวล่าสุด เพื่อป้องกัน QuotaExceededError
        cachedVisits = cachedVisits.slice(0, 50);
    ...
    try {
        if (typeof _supabase !== 'undefined') {
            const { data: existingVisit } = await _supabase.from('visits').select('visit_id, symptom').eq('visit_id', visitId).maybeSingle();
            if (existingVisit) {
                const updatePayload = {
                    meds: medsStr,
                    status: 'รอจ่ายยา'
                };
                if (existingVisit.symptom && existingVisit.symptom !== 'สั่งจ่ายยา') {
                    // ไม่ต้องเขียนทับ symptom เดิม
                } else if (originalSymptom) {
                    updatePayload.symptom = originalSymptom;
                }
    ...
    const mlmItems = (window.currentRxMeds && window.currentRxMeds.length > 0) ? window.currentRxMeds : [];
    if (mlmItems.length > 0) {
        try {
            // ดึงชื่อผู้แนะนำ (Referrer)
            ...
            // 🌟 1. ดึงข้อมูลเบอร์โทรศัพท์ (customer_phone)
            let patientPhone = document.getElementById('rxPatientPhoneDisplay')?.innerText || '-';
            if (patientPhone === '-' && window.allPatients && hn) {
                const p = window.allPatients.find(pat => pat.hn === hn);
                if (p && p.phone) patientPhone = p.phone;
            }

            // 🌟 2. ดึงข้อมูลอาการเบื้องต้น (symptom)
            let patientSymptom = document.getElementById('rxPatientSymptomDisplay')?.innerText || (typeof symptomText !== 'undefined' ? symptomText : '-');

            // 🌟 3. ประกอบ Payload ให้ครบถ้วนตาม Schema ของตาราง stk_nutrient_orders
            const nutrientPayload = {
                order_id: orderId,
                visit_id: visitId || '-',
                hn: hn || 'CLINIC-PATIENT',
                customer_name: patientName || '-',
                customer_phone: patientPhone,           // เบอร์โทรศัพท์
                closer_dr: (typeof doctorName !== 'undefined' ? doctorName : '-'), // แพทย์ผู้ตรวจ
                recorded_by: assistant,                 // ผู้แนะนำ
                symptom: patientSymptom,                // อาการเบื้องต้น
                date: nowIso.split('T')[0],
                status: 'รอดำเนินการ',
                items_json: mlmItems,                   // 🌟 ส่ง Array รายการสินค้าสารอาหารไปลง items_json
                items: mlmItems,
                created_at: nowIso
            };
...
async function saveNutrientOrderToDatabase(salePayload) {
    ...
    let rawItems = [];
    if (Array.isArray(salePayload.items_json)) {
        rawItems = salePayload.items_json;
    } else if (typeof salePayload.items_json === 'string' && salePayload.items_json.trim() !== '') {
        try {
            rawItems = JSON.parse(salePayload.items_json);
        } catch (e) {
    ...
    if (!Array.isArray(rawItems) || rawItems.length === 0) {
        ...
        } else if (typeof salePayload.items === 'string' && salePayload.items.trim() !== '') {
            try {
                rawItems = JSON.parse(salePayload.items);
            } catch (e) { }
        }
    }

    if (!Array.isArray(rawItems) || rawItems.length === 0) {
        if (window.currentRxMeds && Array.isArray(window.currentRxMeds) && window.currentRxMeds.length > 0) {
            rawItems = window.currentRxMeds;
        }
    }

    if (!Array.isArray(rawItems)) rawItems = [];

    // ฟอร์แมตแต่ละรายการให้สมบูรณ์และถูกต้องตามโครงสร้าง
    const cleanItems = rawItems.map(i => {
        ...
        return {
            id: i.id || 'ITEM',
            name: i.name || i.title || i.product_name || 'รายการยา/สารอาหาร',
            type: priceTypeName,
    ...
    let cleanSymptom = salePayload.symptom || null;
    if (cleanSymptom) {
        if (cleanSymptom.includes(' | ความดัน:')) cleanSymptom = cleanSymptom.split(' | ความดัน:')[0];
        else if (cleanSymptom.includes('ความดัน:')) cleanSymptom = cleanSymptom.split('ความดัน:')[0];
        else if (cleanSymptom.includes(' | ชีพจร:')) cleanSymptom = cleanSymptom.split(' | ชีพจร:')[0];
        else if (cleanSymptom.includes('ชีพจร:')) cleanSymptom = cleanSymptom.split('ชีพจร:')[0];
        cleanSymptom = cleanSymptom.replace(/,?\s*ความดัน:.*$/i, '')
            .replace(/,?\s*ชีพจร:.*$/i, '')
            .replace(/,?\s*อุณหภูมิ:.*$/i, '')
            .replace(/,?\s*นน\.?:.*$/i, '')
            .trim();
    }

    // 🌟 ดึงข้อมูล แพทย์ผู้ตรวจ (closer_dr) จาก Payload, visits table หรือ currentUser
    let closerDr = salePayload.closer_dr || salePayload.doctor_name || null;
    if ((!closerDr || closerDr === '-' || closerDr === 'null') && salePayload.visit_id && typeof _supabase !== 'undefined') {
        try {
            const { data: vDb } = await _supabase.from('visits').select('doctor_name').eq('visit_id', salePayload.visit_id).maybeSingle();
            if (vDb && vDb.doctor_name && vDb.doctor_name !== 'null' && vDb.doctor_name !== '-') {
                closerDr = vDb.doctor_name;
            }
        } catch (e) { }
    }
    if ((!closerDr || closerDr === '-' || closerDr === 'null') && window.currentUser) {
        closerDr = window.currentUser.name || window.currentUser.full_name || null;
    }
    ...
    const cleanPayload = {
        ...
        closer_dr: closerDr || null,
        recorded_by: salePayload.recorded_by || 'Staff',
        symptom: cleanSymptom || null,
        date: salePayload.date || (new Date().toISOString().split('T')[0]),
        status: salePayload.status || 'รอดำเนินการ',
        items_json: (cleanItems && cleanItems.length > 0) ? cleanItems : (Array.isArray(salePayload.items_json) ? salePayload.items_json : rawItems)
    };
    ...
    try {
        ...
        if (typeof window.safeSetLocalStorage === 'function') {
            window.safeSetLocalStorage('stk_nutrient_orders', cachedNutrients);
        } else {
...
async function submitPrescriptionToMlm() {
    ...
    const doctorName = document.getElementById('rxDoctorNameDisplay')?.innerText || (typeof window.currentUser !== 'undefined' && window.currentUser ? window.currentUser.name : 'แพทย์ผู้ตรวจ');
    const symptomText = document.getElementById('rxPatientSymptomDisplay')?.innerText || '-';
...
async function loadPharmacyQueue() {
    ...
    data.forEach((row, idx) => {
        ...
        // 2. ปุ่ม จ่ายยาเสร็จสิ้น
        let completeBtn = `
            <button class="btn btn-sm btn-light border w-100 fw-semibold text-success shadow-sm" style="font-size: 0.82rem; padding: 6px 10px; border-radius: 8px;" onclick="completeDispensing('${row.visit_id}')">
        ...
        // 3. ปุ่ม แก้ไขรายการ
        let editBtn = `
            <button class="btn btn-sm btn-light border w-100 fw-semibold text-primary shadow-sm" style="font-size: 0.82rem; padding: 6px 10px; border-radius: 8px;" onclick="editPharmacyPrescription('${row.visit_id}')">
                <i class="bi bi-pencil-square me-1"></i>แก้ไขรายการ
            </button>
        `;

        // 4. นำปุ่มทั้ง 3 มาเรียงต่อกัน โดยเว้นช่องไฟให้พอดี
        const actionBtn = `
            <div class="d-flex flex-column gap-2 align-items-stretch" style="min-width: 140px;">
                ${editBtn}
                ${nutrientBtn}
                ${completeBtn}
            </div>
...
window.viewPharmacyBillDetails = async function (visitId, billType) {
    ...
    // --- 🌟 ดึงข้อมูล แพทย์ผู้ตรวจ และ อาการเบื้องต้น จากฐานข้อมูลให้ตรง 100% ---
    let doctorName = '-';
    let patientSymptom = '-';

    if (visitId && typeof _supabase !== 'undefined') {
        try {
            // ค้นหาข้อมูลจากตาราง visits โดยใช้ visitId ดึงแบบ Real-time
            ...
            console.warn('Fetch pharmacy details error:', e);
        }
    }
    if (doctorName === '-' && visit && visit.doctor_name) doctorName = visit.doctor_name;
...
function isNutrientItem(item) {
    ...
    if (name.includes('STK') || name.includes('BOOM') || name.includes('GOLD') || name.includes('ORYZA') || name.includes('APPLE') || name.includes('COLLAGEN') || name.includes('VIT') || name.includes('COFFEE') || name.includes('PINE') || name.includes('SESAME') || name.includes('ZINC') || name.includes('CORDY')) {
        return true;
    }
...
window.sendPharmacyNutrientOrder = async function (visitId) {
    ...
    // 🌟 ดึงข้อมูล แพทย์ผู้ตรวจ และ อาการเบื้องต้น
    let closerDr = visit.doctor_name || null;
    let patientSymptom = visit.symptom || null;

    if (!closerDr || closerDr === '-' || closerDr === 'null') {
        const docEl = document.getElementById('rxDoctorNameDisplay');
        if (docEl && docEl.innerText && docEl.innerText !== '-') closerDr = docEl.innerText;
    }

    if ((!closerDr || !patientSymptom || closerDr === '-') && typeof _supabase !== 'undefined') {
        try {
            const { data: vDb } = await _supabase.from('visits').select('doctor_name, symptom').eq('visit_id', visitId).maybeSingle();
            if (vDb) {
...
// ฟังก์ชันสำหรับกดยืนยันการจ่ายยาเสร็จสิ้น
window.completeDispensing = async function (visitId) {
    const result = await Swal.fire({
        title: 'ยืนยันจ่ายยา?',
    ...
    try {
        const currentVisit = (window.allPharmacyVisits || []).find(v => v.visit_id === visitId);

        // 1. อัปเดตสถานะเป็น "เสร็จสิ้น" ใน LocalStorage (เพื่อให้หน้าจออัปเดตไวขึ้น)
        try {
            let cachedVisits = JSON.parse(localStorage.getItem('clinic_visits_queue') || '[]');
            if (!Array.isArray(cachedVisits)) cachedVisits = [];
            ...
            const vIdx = cachedVisits.findIndex(v => v.visit_id === visitId);
            if (vIdx !== -1) {
                cachedVisits[vIdx].status = 'เสร็จสิ้น';
                cachedVisits[vIdx].dispensed_at = new Date().toISOString();
                if (currentVisit && currentVisit.meds && !cachedVisits[vIdx].meds) {
                    cachedVisits[vIdx].meds = currentVisit.meds;
                }
            ...
            cachedVisits = cachedVisits.slice(0, 25);
            if (typeof window.safeSetLocalStorage === 'function') {
                window.safeSetLocalStorage('clinic_visits_queue', cachedVisits);
            } else {
                localStorage.setItem('clinic_visits_queue', JSON.stringify(cachedVisits));
            ...
            console.warn('LocalStorage queue update warning:', lsErr);
        }

        // 2. อัปเดตสถานะเป็น "เสร็จสิ้น" ในฐานข้อมูล Supabase
        if (typeof _supabase !== 'undefined') {
            await _supabase
                .from('visits')
                .update({ status: 'เสร็จสิ้น' })
        ...
        Swal.fire({
            icon: 'success',
            title: 'สำเร็จ',
            text: 'จ่ายยาเสร็จสิ้นเรียบร้อยแล้ว',
            timer: 1500,
            showConfirmButton: false
        });

        // 3. รีเฟรชตารางคิวจ่ายยา (ด้านบน) เพื่อให้รายการที่กดแล้วหายไป
        if (typeof loadPharmacyQueue === 'function') {
            await loadPharmacyQueue();
        }

        // 4. รีเฟรชตารางประวัติการจ่ายยา (ด้านล่าง) เพื่อแสดงรายการที่เพิ่งจ่ายเสร็จ
        ...
        console.error('Error in completeDispensing:', e);
...
async function showHistoryDetails(visitId) {
    ...
    // 🌟 ดึงข้อมูล อาการแรกรับ / อาการสำคัญ ที่แท้จริง (หลีกเลี่ยงข้อความสถานะ เช่น 'สั่งจ่ายยา', 'รอจ่ายยา')
    const invalidSymptoms = ['สั่งจ่ายยา', 'รอจ่ายยา', 'รอตรวจ', 'เสร็จสิ้น', 'สำเร็จ', 'รอรับยา', 'กำลังตรวจ'];
    let realSymptom = row.symptom || '';

    if (invalidSymptoms.includes(realSymptom.trim())) {
        realSymptom = row.initial_symptom || row.triage_symptom || row.chief_complaint || '';
    }

    if ((!realSymptom || invalidSymptoms.includes(realSymptom.trim())) && row.hn) {
        const matchingVisit = (window.clinicVisits || window.allVisits || []).find(v =>
            (v.visit_id === row.visit_id || v.hn === row.hn) &&
            v.symptom && !invalidSymptoms.includes(v.symptom.trim())
        );
        if (matchingVisit) realSymptom = matchingVisit.symptom.trim();
    }

    if ((!realSymptom || invalidSymptoms.includes(realSymptom.trim())) && row.hn && window.allPatients) {
        const pat = window.allPatients.find(p => p.hn === row.hn);
        if (pat && pat.symptom && !invalidSymptoms.includes(pat.symptom.trim())) {
            realSymptom = pat.symptom.trim();
        }
    }

    setSafeText('histSymptom', (realSymptom && !invalidSymptoms.includes(realSymptom.trim())) ? realSymptom : 'ไม่มีระบุ');
...
function filterStaffTable() {
    ...
    if (list && list.length > 0 && typeof renderStaffTable === 'function') {
        const filtered = list.filter(u => {
            const empCode = String(u.emp_code || u.code || u.Employee_ID || '').toLowerCase();
            const name = String(u.full_name || u.name || '').toLowerCase();
            const email = String(u.email || '').toLowerCase();
            const role = String(u.role || '').toLowerCase();

            const cleanCode = empCode.replace(/[-\s]/g, '');
            const cleanName = name.replace(/[-\s]/g, '');
            const cleanEmail = email.replace(/[-\s]/g, '');

            return empCode.includes(q) || (cleanQ && cleanCode.includes(cleanQ)) ||
                name.includes(q) || (cleanQ && cleanName.includes(cleanQ)) ||
                email.includes(q) || (cleanQ && cleanEmail.includes(cleanQ)) ||
                role.includes(q);
        });
        ...
        rows.forEach(row => {
            ...
            row.style.display = (!q || rowText.includes(q) || (cleanQ && cleanRowText.includes(cleanQ))) ? '' : 'none';
        });
    }
}
...
async function submitStaffUser() {
    ...
    let editId = editIdInput ? editIdInput.value.trim() : '';
    ...
    try {
        ...
        let error;
        // ถ้าเป็นการสร้างใหม่ ให้เช็คก่อนว่ามี emp_code นี้อยู่ในระบบแล้วหรือไม่ เพื่ออัปเดตแทนการเพิ่มซ้ำ
        if (!editId) {
            const { data: existing } = await _supabase.from('staff_users').select('id').eq('emp_code', empCode).maybeSingle();
            if (existing && existing.id) {
                editId = existing.id; // สลับเป็นโหมดอัปเดตพนักงานคนเดิมอัตโนมัติ
            }
        }
        ...
        if (error && error.message && (error.message.includes('staff_users_emp_code_key') || error.message.includes('duplicate key'))) {
            const { data: existing } = await _supabase.from('staff_users').select('id').eq('emp_code', empCode).maybeSingle();
            if (existing && existing.id) {
                const updateRes = await _supabase.from('staff_users').update(payload).eq('id', existing.id);
        ...
        await loadStaffUsers();
        Swal.fire({ icon: 'success', title: editId ? 'บันทึกการแก้ไขสำเร็จ!' : 'เพิ่มข้อมูลพนักงานสำเร็จ!', timer: 1800, showConfirmButton: false });
    } catch (err) {
        console.error('submitStaffUser error:', err);
        let msg = err.message || 'ไม่สามารถบันทึกข้อมูลได้';
        if (msg.includes('staff_users_emp_code_key') || msg.includes('duplicate key')) {
            msg = `รหัสพนักงาน "${empCode}" นี้มีอยู่ในระบบแล้ว กรุณาใช้รหัสพนักงานอื่น หรือเลือกแก้ไขข้อมูลพนักงานคนเดิม`;
        }
...
// ===============================================
// Helper แปลง Date เป็น YYYY-MM-DD ตาม Local Time
function getExpenseLocalDateStr(dateInput) {
    if (!dateInput) return '';
    const d = typeof dateInput === 'string' && dateInput.length === 10 && dateInput.includes('-')
...
async function loadExpenses() {
    try {
        ...
        // ถ้ามีการกรองค้างอยู่ให้กรองตามค่านั้น ถ้าไม่มีให้เรนเดอร์ทั้งหมด
        const q = document.getElementById('expenseSearchInput')?.value?.trim();
        const cat = document.getElementById('expenseCategoryFilter')?.value?.trim();
        const sDate = document.getElementById('expenseStartDate')?.value?.trim();
        const eDate = document.getElementById('expenseEndDate')?.value?.trim();
...
function updateExpenseStats(list) {
    ...
    let monthTotal = 0;
    list.forEach(item => {
        const itemDate = item.date || (item.created_at ? getExpenseLocalDateStr(item.created_at) : '');
        const amount = parseFloat(item.amount || 0);
...
function filterExpensesTable() {
    ...
    const cat = (document.getElementById('expenseCategoryFilter')?.value || '').trim();
    const startDateStr = document.getElementById('expenseStartDate')?.value || '';
    const endDateStr = document.getElementById('expenseEndDate')?.value || '';

    let filtered = (window.clinicExpensesData || []).filter(item => {
        const rowDate = item.date || (item.created_at ? getExpenseLocalDateStr(item.created_at) : '');
        ...
        let passDate = true;
        if (startDateStr || endDateStr) {
            if (startDateStr && rowDate < startDateStr) passDate = false;
            if (endDateStr && rowDate > endDateStr) passDate = false;
        }
...
function resetExpenseFilter() {
    ...
    if (document.getElementById('expenseStartDate')) document.getElementById('expenseStartDate').value = '';
...
async function saveExpense(e) {
    ...
    const dateVal = dateInput?.value || getExpenseLocalDateStr(new Date());
...
function openExpenseModal() {
    try {
        ...
        const dateInput = document.getElementById('expenseDate');
        if (dateInput) {
            const today = getExpenseLocalDateStr(new Date());
...
// Helper สำหรับกำหนดช่วงเวลาด่วนในหน้าสรุปยอดคลินิกรายวัน
...
window.setExpenseDateFilter = setExpenseDateFilter;
...
// ==========================================
// ส่วนของการพิมพ์และ Export สำหรับหน้า "รายจ่ายประจำวัน" (Expenses)
// ==========================================
...
window.printExpensesReport = function () {
    ...
    const signaturesHtml = `
        <div class="signatures">
            <div class="sig-box">
                <div class="sig-line"></div>
                <div>( ເຈົ້າໜ້າທີ່ການເງິນ / ຜູ້ຈັດທຳ )</div>
                <div style="font-size: 12px; color: #666;">( เจ้าหน้าที่การเงิน / ผู้จัดทำ )</div>
            </div>
            <div class="sig-box">
                <div class="sig-line"></div>
                <div>( ຜູ້ຈັດການ / ຜູ້ກວດສອບ )</div>
                <div style="font-size: 12px; color: #666;">( ผู้จัดการ / ผู้ตรวจสอบ )</div>
            </div>
        </div>
    `;
...
// 3. ฟังก์ชัน Export PDF
window.exportExpensesPDF = function () {
    // บนเบราว์เซอร์ปัจจุบัน การสั่ง Print สามารถเลือก "Save as PDF" ได้ 
    // เราจึงใช้ฟังก์ชัน Print ร่วมกันเพื่อความเสถียรและสวยงามที่สุดครับ
    printExpensesReport();
};
// ฟังก์ชันสำหรับกำหนดช่วงเวลาด่วนในหน้าใบสรุปยอดคลินิก
window.setClinicSummaryDateFilter = function (mode) {
    const startInput = document.getElementById('dailyReportStartDate');
    const endInput = document.getElementById('dailyReportEndDate');
    const now = new Date();
...
window.setPharmacyHistoryDateFilter = function (type) {
    const startEl = document.getElementById('pharmacyHistoryStartDate');
    const endEl = document.getElementById('pharmacyHistoryEndDate');
...
// // ฟังก์ชันโหลดประวัติการจ่ายยา (ปรับปรุงให้อ่านข้อมูลทันทีจาก LocalStorage & Supabase พร้อมรองรับตัวกรองวันที่)
window.loadPharmacyHistory = async function () {
    ...
    // 1. ดึงข้อมูลจากฐานข้อมูล Supabase ก่อน
    try {
        if (typeof _supabase !== 'undefined') {
            let q = _supabase
                .from('visits')
            ...
            if (startFilter) {
                q = q.gte('created_at', startFilter + 'T00:00:00');
            }
            if (endFilter) {
        ...
        console.warn('Load pharmacy history DB notice:', e);
    }

    // 2. 🌟 ดึงข้อมูลจาก LocalStorage มารวมด้วย (ช่วยให้โชว์ทันทีที่กดปุ่มเสร็จสิ้น!)
    try {
        const cachedVisits = JSON.parse(localStorage.getItem('clinic_visits_queue') || '[]');
        if (Array.isArray(cachedVisits)) {
            // กรองเอาเฉพาะเคสที่ 'เสร็จสิ้น'
            const completedCached = cachedVisits.filter(v => {
                if (v.status !== 'เสร็จสิ้น' || !v.meds || v.meds === '[]') return false;
                if (startFilter || endFilter) {
                    const rowDate = (v.dispensed_at || v.created_at || '').split('T')[0];
                    if (rowDate) {
                        if (startFilter && rowDate < startFilter) return false;
                        if (endFilter && rowDate > endFilter) return false;
                    }
                }
                return true;
            });

            // นำข้อมูลมารวมกันโดยไม่ให้ซ้ำ (เช็คจาก visit_id)
            const map = new Map();
            historyData.forEach(v => map.set(v.visit_id, v));
            completedCached.forEach(v => {
                const existing = map.get(v.visit_id) || {};
                map.set(v.visit_id, { ...existing, ...v });
            });

            // แปลงกลับเป็น Array และสลับให้รายการล่าสุดอยู่ด้านบน
            historyData = Array.from(map.values()).reverse();
        }
    } catch (e) {
        console.warn('Load local pharmacy history error:', e);
    }

    // เริ่มวาดตาราง
    tbody.innerHTML = '';

    // กรองอีกครั้งเพื่อให้แน่ใจว่ามียาจริงๆ และตรงตามช่วงวันที่
    const validData = historyData.filter(row => {
        if (!row.meds || row.meds === '[]' || row.meds === '""') return false;
        if (startFilter || endFilter) {
            const rowDate = (row.dispensed_at || row.created_at || row.date || '').split('T')[0];
            if (rowDate) {
                if (startFilter && rowDate < startFilter) return false;
                if (endFilter && rowDate > endFilter) return false;
            }
    ...
    tbody.innerHTML = validData.map((row, idx) => {
        ...
        try {
            ...
            medsHtml = medsList.map(m => {
                let cleanName = m.name || m.product_name || 'รายการยา';
                cleanName = cleanName.replace(' (โปร)', '').replace(' (ส่ง/สมาชิก)', '').replace(' (แถมฟรี)', '');
                return `<span class="badge bg-light text-dark border border-secondary-subtle me-1 mb-1 fw-medium px-2 py-1"><i class="bi bi-check2 text-success me-1"></i>${cleanName} <strong class="text-primary ms-1">x ${m.qty || 1}</strong></span>`;
            }).join('');
        } catch (e) {
            medsHtml = '<span class="text-danger small">ไม่สามารถแสดงข้อมูลยาได้</span>';
        }
        ...
        return `
            <tr>
                ...
                <td class="align-middle py-3">${medsHtml || '-'}</td>
                <td class="text-center align-middle">
                    <span class="badge bg-success-subtle text-success border border-success-subtle px-3 py-1.5" style="font-size: 0.8rem;"><i class="bi bi-check-circle-fill me-1"></i>จ่ายยาแล้ว</span>
                </td>
            </tr>
        `;
    }).join('');
};
// ฟังก์ชันสำหรับกำหนดช่วงเวลาด่วนในหน้าประวัติห้องจ่ายยา
...
// =====================================
// ระบบประวัติจุดคัดกรอง (Triage History) - รองรับ Search + Phone + Pagination
// =====================================
    ...
    try {
        if (typeof _supabase !== 'undefined') {
            ...
            try {
                ...
                console.warn('Map patient phone to triage history notice:', err);
            }

            window.triageHistoryData = rows;
        }
    } catch (e) {
        console.warn('Load triage history error:', e);
    }

    // เมื่อโหลดข้อมูลเสร็จ ให้เรียกฟังก์ชันค้นหา (เผื่อมีคำที่พิมพ์ค้างไว้)
    filterTriageHistory();
};
...
window.filterTriageHistory = function () {
    ...
    if (!query) {
        window.triageHistoryFilteredData = [...window.triageHistoryData];
    } else {
        window.triageHistoryFilteredData = window.triageHistoryData.filter(row => {
            const hn = (row.hn || '').toLowerCase();
            const name = (row.patient_name || '').toLowerCase();
            const visitId = (row.visit_id || '').toLowerCase();
            const phone = (row.phone || row.tel || row.emergency_tel || '').toLowerCase();
            ...
            // ค้นหาเจอตัวใดตัวหนึ่งก็ให้แสดง (ค้นหาเบอร์แบบมีหรือไม่มีขีดได้)
            return hn.includes(query) ||
                name.includes(query) ||
                visitId.includes(query) ||
                symptom.includes(query) ||
                (phone && phone.includes(query)) ||
                (cleanPhone && cleanPhone.includes(cleanQuery));
        });
    }

    // รีเซ็ตหน้ากลับไปที่ 1 ทุกครั้งที่มีการพิมพ์ค้นหา
    window.triageHistoryCurrentPage = 1;
    renderTriageHistoryTable();
};
...
window.renderTriageHistoryTable = function (page = window.triageHistoryCurrentPage) {
    ...
    currentData.forEach(row => {
        let vitals = [];
        if (row.temp) vitals.push(`<span class="badge bg-light text-dark border border-secondary-subtle me-1 mb-1 shadow-sm px-2 py-1">Temp: ${row.temp}°C</span>`);
        if (row.bp) vitals.push(`<span class="badge bg-light text-dark border border-secondary-subtle me-1 mb-1 shadow-sm px-2 py-1">BP: ${row.bp}</span>`);
        if (row.pulse) vitals.push(`<span class="badge bg-light text-dark border border-secondary-subtle me-1 mb-1 shadow-sm px-2 py-1">Pulse: ${row.pulse}</span>`);
        ...
        tbody.innerHTML += `
            <tr class="border-bottom">
                ...
                <td class="text-center align-middle py-2">
                    <div class="mb-2">
                        <button class="btn btn-sm btn-outline-primary px-3 py-1 fw-semibold shadow-sm" 
                            style="font-size: 0.75rem; border-radius: 50rem;" 
                            onclick="openTriageModal('${row.visit_id}')">
                            <i class="bi bi-pencil-square me-1"></i> ซักประวัติ / แก้ไข
                        </button>
                    </div>
                    <span class="badge bg-success-subtle text-success border border-success-subtle px-3 py-1" style="font-size: 0.75rem;"><i class="bi bi-check-circle-fill me-1"></i>คัดกรองแล้ว</span>
                    <div class="text-muted fw-semibold mt-1" style="font-size: 0.72rem;">(สถานะ: ${row.status})</div>
                </td>
            </tr>
        `;
    });

    // 🌟 ส่วนที่ถูกปรับปรุง: วาดปุ่ม Pagination พร้อมข้อความบอกจำนวนรวมที่มุมขวา
    if (paginationContainer) {
        // เคลียร์คลาสเก่าที่อาจทำให้การจัดหน้าเพี้ยน
        paginationContainer.className = "mt-3 mb-2";
        ...
        if (totalPages > 1) {
            ...
            for (let i = 1; i <= totalPages; i++) {
                if (i === 1 || i === totalPages || (i >= window.triageHistoryCurrentPage - 2 && i <= window.triageHistoryCurrentPage + 2)) {
                    ...
                    paginationHtml += `<button class="btn btn-sm ${activeClass} px-3 py-1" onclick="renderTriageHistoryTable(${i})">${i}</button>`;
                } else if (i === window.triageHistoryCurrentPage - 3 || i === window.triageHistoryCurrentPage + 3) {
                    paginationHtml += `<span class="px-2 text-muted">...</span>`;
                }
            }

            paginationHtml += `<button class="btn btn-sm btn-light border text-muted px-2.5 py-1" onclick="renderTriageHistoryTable(${window.triageHistoryCurrentPage + 1})" ${window.triageHistoryCurrentPage === totalPages ? 'disabled' : ''}><i class="bi bi-chevron-right"></i></button>`;
        }
        paginationHtml += `</div>`;
